...
const { Subscription, Transaction, PLAN_TYPES, PLAN_CONFIG } = require('../models/Subscription');
...
const createOrder = async (userId, plan) => {
  if (!Object.values(PLAN_TYPES).includes(plan)) {
    throw ApiError.badRequest(`Invalid plan. Choose 'single' (₹50) or 'bundle' (₹400)`);
  }
...
const verifyPaymentAndActivate = async (userId, paymentData) => {
  ...
  emailService.sendSubscriptionConfirmationEmail(user, subscription, transaction).catch((err) =>
    logger.error(`Subscription email failed: ${err.message}`)
  );

  logger.info(`Payment verified & subscription activated: ${subscription._id} for user ${userId}`);
...
const handleWebhook = async (rawBody, signature, payload) => {
  ...
  if (event === 'payment.captured') {
    ...
    if (!existing) {
      logger.info(`Webhook fallback: processing payment for order ${payment.order_id}`);
...
const getUserTransactions = async (userId, { page = 1, limit = 20 }) => {
  ...
  const [transactions, total] = await Promise.all([
    Transaction.find({ user: userId })
      ...
      .skip(skip)
      .limit(Number(limit))
      .populate('subscription', 'plan totalCredits usedCredits status expiresAt')
